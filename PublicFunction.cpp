...
//    {
//        qSqlDataBase = QSqlDatabase::database("qt_sql_default_connection");
//    }
//    else
//    {
...
void CFuseImageThread::run()
{
    ...
    iImageCount = m_strResultImagePathList.count();
    iImageWidth = m_iImageWidth / iImageCount;
    for(int i = 0; i < iImageCount; ++i)
    {
        painter.drawPixmap(i * iImageWidth, 0, iImageWidth, m_iImageHeigth, QPixmap(m_strResultImagePathList.at(i)));
    }
    m_pPixmap->save(m_strSaveResultImagePath);
    emit SignalFuseOk();
    qDebug() << "SignalFuseOk();";
...
void CFuseImageThread::SetImagePaths(QStringList strImagePathList, QString strSaveImagePath,
                                     QStringList strResultImagePathList,QString strSaveResultImagePath)
{

    m_strImagePathList = strImagePathList;
    m_strSaveImagePath = strSaveImagePath;
    m_strResultImagePathList = strResultImagePathList;
    m_strSaveResultImagePath = strSaveResultImagePath;
