...
QRCodeDetector::~QRCodeDetector()
{
    qDebug() << "~QRCodeDetector()";
    if(m_pZxingDecoder != NULL)
    {
        delete m_pZxingDecoder;
    }
    qDebug() << "~QRCodeDetector()123";
}
...
void QRCodeDetector::_SlotGetQRcode()
{
    ...
    {
        emit SignalErrInfo(ErrNoFoundQR);
    }
    else
    {
        //解析二维码
        ...
        {
            emit SignalErrInfo(ErrDecodeQR);
        }
        else
        {
...
bool QRCodeDetector::InitDevice()
{
    ...
    //开下白灯
    CHidCmdThread::GetInstance()->AddOpenLedCmd(2,m_Params.TCup_pas.TCupBasicSaveParams.Light2);
    HIDOpertaionUtility::GetInstance()->SetDeviceOperate(true);
    while (HIDOpertaionUtility::GetInstance()->GetDeviceOperateStates())
    {
        QApplication::processEvents();
...
bool QRCodeDetector::locationQRCode(QString &strQRCodeInfo,qint32 &iQRCodePosition)
{
    ...
    //顺时针转 40 * 10 步
    for(qint16 iStep1 = 0;iStep1 < 5;iStep1++)
    {
        HIDOpertaionUtility::GetInstance()->SetDeviceOperate(true);
        CHidCmdThread::GetInstance()->AddRotateMotorCmd(20,60,0);
        ...
        if(GetQRCodeImageInfo(strDesImage,strQRCodeInfo) == true)
        {
            this->SetQRCodePosition(4096 - iStep1 * 60);
            iQRCodePosition = GetQRCodePosition();
            qDebug() << "strQRCodeInfo = " << strQRCodeInfo;
            qDebug() << "QRCodePosition = " << iQRCodePosition;
            return true;
    ...
    //转动复位位置
    HIDOpertaionUtility::GetInstance()->SetDeviceOperate(true);
    CHidCmdThread::GetInstance()->AddRotateMotorCmd(20,240,1);
    ...
    //逆时针转 40 * 10 步
    for(qint16 iStep2 = 0;iStep2 < 5;iStep2++)
    {
        HIDOpertaionUtility::GetInstance()->SetDeviceOperate(true);
        CHidCmdThread::GetInstance()->AddRotateMotorCmd(20,60,1);
        ...
        if(GetQRCodeImageInfo(strDesImage,strQRCodeInfo) == true)
        {
            this->SetQRCodePosition(iStep2 * 60);
            iQRCodePosition = GetQRCodePosition();
            qDebug() << "strQRCodeInfo = " << strQRCodeInfo;
            qDebug() << "QRCodePosition = " << iQRCodePosition;
            return true;
...
{
    bool bIsGetImage = false;
    for(int iPos = 0;iPos < 3;iPos++)
    {
        bIsGetImage = OpencvUtility::GetInstance()->GetVideoCapture(&strImagePath);
        if(bIsGetImage == true)
        {
            break;
        }
...
//    //从文件中读取图像
//        IplImage *pSrcImage = cvLoadImage((const char *)strImageDecodePath.toLocal8Bit(), CV_LOAD_IMAGE_UNCHANGED);
//        IplImage *pDstImage = NULL;

//        CvSize czSize;              //目标图像尺寸

//        //计算目标图像大小
//        czSize.width = pSrcImage->width * 0.5;
//        czSize.height = pSrcImage->height * 0.5;

//        //创建图像并缩放
//        pDstImage = cvCreateImage(czSize, pSrcImage->depth, pSrcImage->nChannels);
//        cvResize(pSrcImage, pDstImage, CV_INTER_AREA);
//        //保存图片
//        cvSaveImage((const char *)strImageDecodePath.toLocal8Bit(), pDstImage);
            ...
            if(pZXing != NULL)
            {
                for(int iPos = 0;iPos < 3;iPos++)
                {
                    try
                    {
                        ...
                        strQRCodeInfo = pZXing->decodeImage(qImage,qImage.width(),qImage.height(),true);
                    }
                    catch(...) {}
...
bool QRCodeDetector::ExtractQRCode(QString strSrcImage,QString &strDesImage)
{
    ...
    //IplImage *pCopyImage= NULL;
    //IplImage *pCopyImage1 = NULL;
    ...
    try
    {
        ...
        IplImage *pSrcImage = cvLoadImage((const char *)strSrcImage.toLocal8Bit(),1);
        if(pSrcImage == NULL)
        {
            return false;
            //goto RelseaseRes;
        ...
        //转变为灰度图
        if(cvGetSize(pSrcImage).height <= 0 || cvGetSize(pSrcImage).width <= 0)
        {
            if(pSrcImage != NULL)
            {
        ...
        if(cvGetSize(pGrayimage).height <= 0 || cvGetSize(pGrayimage).width <= 0)
        {
            if(pSrcImage != NULL)
            {
               cvReleaseImage(&pSrcImage);
        ...
        //qDebug() << __FUNCTION__ << __LINE__;
        if(cvGetSize(pSobel).height <= 0 || cvGetSize(pSobel).width <= 0)
        {
            if(pSrcImage != NULL)
            {
               cvReleaseImage(&pSrcImage);
        ...
        //qDebug() << __FUNCTION__ << __LINE__;
        //对图像进行二值化处理
        if(cvGetSize(pTemp).height <= 0 || cvGetSize(pTemp).width <= 0)
        {
            if(pSrcImage != NULL)
            {
               cvReleaseImage(&pSrcImage);
    ...
    //    cvThreshold(pTemp,pThreshold1,10,80,CV_THRESH_BINARY/*| CV_THRESH_OTSU*/);
        //自适应阈值方法
        cvAdaptiveThreshold( pTemp, pThreshold1 , 255, CV_THRESH_BINARY , CV_ADAPTIVE_THRESH_GAUSSIAN_C , 7/*11*/, 3 );
        //qDebug() << __FUNCTION__ << __LINE__;
    ...
    //    cvWaitKey(0);
        ...
        //自定义1*3的核进行X方向的膨胀腐蚀
        if(cvGetSize(pThreshold1).height <= 0 || cvGetSize(pThreshold1).width <= 0)
        {
            return false;
        }
        ...
        cvErode(pErode_dilate, pErode_dilate, pKernal, 4);// Y方向腐蚀去除碎片

        cvDilate(pErode_dilate, pErode_dilate, pKernal, 2);//回复形态

        if(pKernal != NULL)
        {
            cvReleaseStructuringElement(&pKernal);
        }

//        cv::namedWindow( "1", cv::WINDOW_NORMAL );
//        cvShowImage("1", pErode_dilate);
//        cvWaitKey(0);

        //图形检测
        if(cvGetSize(pErode_dilate).height <= 0 || cvGetSize(pErode_dilate).width <= 0)
        {
            if(pSrcImage != NULL)
            {
               cvReleaseImage(&pSrcImage);
        ...
        IplImage *pCopyImage = cvCreateImage(cvGetSize(pErode_dilate),pErode_dilate->depth, pErode_dilate->nChannels);
        //qDebug() << __FUNCTION__ << __LINE__;
        if(pCopyImage == NULL)
        {
            if(pSrcImage != NULL)
        ...
        //pCopyImage = cvCloneImage(pErode_dilate);//直接把erode_dilate的数据复制给copy
        cvCopy(pErode_dilate,pCopyImage,NULL);
        //qDebug() << __FUNCTION__ << __LINE__;
        if(pCopyImage == NULL)
        {
            if(pSrcImage != NULL)
        ...
        if(cvGetSize(pSrcImage).height <= 0 || cvGetSize(pSrcImage).width <= 0)
        {
            ...
            if(pCopyImage != NULL)
            {
               cvReleaseImage(&pCopyImage);
               pCopyImage = NULL;
            }
            return false;
        }
        IplImage *pCopyImage1 = cvCreateImage(cvGetSize(pSrcImage),pSrcImage->depth, pSrcImage->nChannels);
        //qDebug() << __FUNCTION__ << __LINE__;
        if(pCopyImage1 == NULL)
        {
            if(pSrcImage != NULL)
        ...
        //pCopyImage1 = cvCloneImage(pSrcImage);//直接把image的数据复制给copy1
        cvCopy(pSrcImage,pCopyImage1,NULL);
        //qDebug() << __FUNCTION__ << __LINE__;
        if(pCopyImage1 == NULL)
        {
            if(pSrcImage != NULL)
        ...
        cvFindContours(pCopyImage, pStorage, &pContoursSeq);  //寻找轮廓
        //qDebug() << __FUNCTION__ << __LINE__;
        ...
        while(pContoursSeq != NULL)
        {
            ...
            */
            ...
            if(cvRect.width / cvRect.height > 0.8
                ...
                &&cvRect.height * cvRect.height * 15 > pCopyImage1->height * pCopyImage1->width
                &&cvRect.y < pCopyImage1->height - 50
                &&cvRect.y > 100
                &&((cvRect.x + cvRect.width) < (pCopyImage->width-100) && (cvRect.y + cvRect.height) < (pCopyImage->height-100))
                )
            {
        ...
        if(pContoursSeq != NULL)
        {
            cvReleaseMemStorage(&pContoursSeq->storage);
        }
        ...
        if(!qRectVector.isEmpty())
        {
            ...
            for(int iPos = 0;iPos < qRectVector.count();iPos++)
            {
                ...
                if(pSrcImage->width < qRectVector.at(iPos).width || pSrcImage->height < qRectVector.at(iPos).height
                   || (qRectVector.at(iPos).x + qRectVector.at(iPos).width) > pSrcImage->width
                   || (qRectVector.at(iPos).y + qRectVector.at(iPos).height) > pSrcImage->height)
                {
                    continue;
                }
                ...
                IplImage *pDstImg = cvCreateImage(cvSize(qRectVector.at(iPos).width,qRectVector.at(iPos).height),
                                       pSrcImage->depth,
                                       pSrcImage->nChannels);
...
//                }
                ...
                //qDebug()  << "strSrcImage = " << strSrcImage;
                qint16 iPos1 = strSrcImage.lastIndexOf(".");
                //qDebug() << "iPos1 = " << iPos1;
                QString strDesImage1 = strSrcImage.insert(iPos1,"QR");
                cvSaveImage((const char *)strDesImage1.toLocal8Bit(),pDstImg);
...
{ 
    cv::Mat mMyMat = cv::cvarrToMat(pDstImg);
    cv::Mat mImage = mMyMat;
    if (mMyMat.empty())
    {
    ...
    //************************************************************
    cv::cvtColor(mMyMat, mImage, CV_BGR2GRAY);
    cv::threshold(mImage, mImage, 100, 255, cv::THRESH_BINARY);
    cv::medianBlur(mImage, mImage, 3);
    //imshow("new", image);
    //waitKey(0);
    cv::Mat mEdges = mImage;
    //边缘检测
    cv::Canny(mImage, mEdges, 100, 255, 3);
    //imshow("edges", edges);
    //waitKey(0);
    ...
    //寻找轮廓
    cv::findContours(mEdges, contoursVector, hierarchyVector, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE);
...
//    }
    ...
    cv::imwrite(string((const char *)strPath.toLocal8Bit()),mMyMat);
            ...
            else if(strVersion == "V5") //V5版本
            {
                ...
                if(strListitemVector.count() < iAllCount)
                {
                    ...
                    else   //项目（0 - 24）
                    {
                        ...
                        strListitem.append(strItem.mid(2,2));
                        strListitem.append(strItem.mid(4,strItem.length() - 4));
                        strListitemVector.append(strListitem);
                    }
                }
                                             ...
                                             QVector<QStringList> strListitemVector)
{
    ...
    {
    case 0:
        sQrCodeInfo.eTypeCup = TypeTCup;
        break;
    case 1:
        sQrCodeInfo.eTypeCup = TypeKCup5;
        break;
    case 4:
        sQrCodeInfo.eTypeCup = TypeSCup10;
        break;
    case 6:
        sQrCodeInfo.eTypeCup = TypeKCup6;
        break;
    default:
        sQrCodeInfo.eTypeCup = TypeTCup;
        break;
    ...
    if(sQrCodeInfo.strVerson == "V5")
    {
        for(int iPos = 0;iPos < strListitemVector.count();iPos++)
        {
            if(strListitemVector.at(iPos).count() >= 3)
            {
                ...
                sInfoProject.strProjectName = GetProjectName(sInfoProject.iIndexProgram - 1);
                sInfoProject.dThresholdUp = dThresholdUp.toInt(&bOk,16) * 0.001;
                sInfoProject.dSensitivityUp = GetV5CutOffValue(sQrCodeInfo.eTypeCup,
                                                               strIndexProgram.toInt(&bOk,16) - 1,
                                                               dSensitivityUp.toInt(&bOk,16));
                if(strListitemVector.at(iPos).count() >= 5)
                {
                    ...
                    sInfoProject.dSensitivityDown = GetV5CutOffValue(sQrCodeInfo.eTypeCup,
                                                                   strIndexProgram.toInt(&bOk,16) - 1,
                                                                     dSensitivityDown.toInt(&bOk,16));
                }
                else
                {
                    //没有下阈值 下灵敏度 则填 0.0
    ...
    else //V2 、V4
    {
        for(int iPos = 0;iPos < strListitemVector.count();iPos++)
        {
            if(strListitemVector.at(iPos).count() >= 3)
            {
                ...
                sInfoProject.dThresholdUp = GetV4threshold(dThresholdUp.toInt(&bOk,16));
                ...
                if(strListitemVector.at(iPos).count() >= 5)
                {
                    dThresholdDown = strListitemVector.at(iPos).at(3);
                    dSensitivityDown = strListitemVector.at(iPos).at(4);
                    sInfoProject.dThresholdUp = GetV4threshold(dThresholdDown.toInt(&bOk,16));
...
{
    const QString ProjectNameArray[39] = {"MOP","MET","KET","BZO","COC","MDMA","AMP","BAR","TCA","OXY","BUP",
                                         "THC","PCP","MTD","OPI","PPX","CAF","COT","EDDP","ETG","FTY","K2",
                                         "TRA","MQL","MC","mAMP","","","","PCP-MDMA","COC-AMP","BAR_MTD",
                                         "COC_MET","MET_THC","MOP_MET","OXY_THC","OPI_BZO","TCA_BUP","TRA_K2"};
    QString strProjectName = "";
    qDebug() << "iIndex = " << iIndex;
    if(iIndex > -1 && iIndex < 39)
    {
        strProjectName = ProjectNameArray[iIndex];
    }
...
int QRCodeDetector::GetV5CutOffValue(const int iCupType,const int iIndex,const int iCutOff)
{
    ...
    qUrineVector << 300 << 1000 << 1000 << 300 << 300 << 500 << 1000 << 300 << 1000 << 100 << 10 << 50 << 25 << 300
                 << 2000 << 300 << 8 << 100 << 100 << 500 << 20 << 500 << 1000 << 300 << 1000 << 1000;
    QVector<int> qSalivaVector;
    qSalivaVector << 15 << 50 << 100 << 30 << 20 << 100 << 40 << 60 << 0 << 20 << 5 << 25 << 10 << 30 << 15 << 0 << 0
                  << 0 << 20 << 0 << 0 << 0 << 0 << 0 << 0 << 50;
...
/**
* @brief  GetV4threshold
* 计算V4版本阈值
* @param iValue 二维码上的值
* @return 解析后的阈值
*/
double QRCodeDetector::GetV4threshold(int iValue)
{
    const double V4thresholdArray[10] = {0.02,0.025,0.03,0.035,0.04,0.045,0.05,0.06,0.07,0.08};
    double V4threshold = 0.0;
    if((iValue > -1) && (iValue < 10))
    {
        V4threshold = V4thresholdArray[iValue];
    }
    return V4threshold;
}
