...
//    cvRectangle(plSrcImg,cvPoint(cvRect.x,cvRect.y),cvPoint(cvRect.x + cvRect.width,cvRect.y + cvRect.height),cv::Scalar(255,0,0),1,1,0);
//    cv::namedWindow( "donkeyaime1", cv::WINDOW_NORMAL );
//    cvShowImage("donkeyaime1",plSrcImg);
//    cv::waitKey(0);
    ...
    dStandardSD = sqrt(dAccum/(iIntensityVector.count()-1));
...
* @return
*/
bool CalculateGreenComponent::GetGrayscaleComponentSDAvg(QString strImagePath,QPoint qPoint,int iRectR,
                                                     int iRectD,int iPos,double dStandardValue,
                                                     double &dAvg,double &dStandardSD)
{
    IplImage *plSrcImg = cvLoadImage( strImagePath.toLatin1().data(), 0 );
    IplImage *plDstImg = NULL;
    CvRect cvRect = GetRect(qPoint,iRectR,iRectD,iPos);
...
//    cv::waitKey(0);
    ...
    for (int iRow = 0; iRow < plDstImg->height; iRow++)
    {
       for (int iCols = 0; iCols < plDstImg->width; iCols++)
       {
           pPixel = reinterpret_cast <uchar*> (plDstImg->imageData + iRow * plDstImg->widthStep)[iCols];
           iIntensityVector.append(pPixel);
       }
    }
    double dAccum  = 0.0;
...
* @return
*/
void CalculateGreenComponent::RrawRect(cv::Mat &Mimage,QPoint qPoint,int iRectR,int iRectD,int iPos)
{
    //获取矩形大小和位置
    CvRect cvRect = GetRect(qPoint,iRectR,iRectD,iPos);
    //画矩形
    cv::rectangle(Mimage,cvRect,cv::Scalar(255,0,0),1,1,0);
...
CvRect CalculateGreenComponent::GetRect(QPoint qPoint,int iRectR,int iRectD,int iPos)
{
    ...
    {
    case 0:
        iRowBegin =  qPoint.y() - iRectR;
        iColsBegin = qPoint.x() - iRectR - iRectD;
        break;
    case 1:
        iRowBegin =  qPoint.y() - iRectR -  2 * iRectD;
        iColsBegin = qPoint.x() - iRectR - iRectD;
        break;
    case 2:
        iRowBegin =  qPoint.y() - iRectR - 4 * iRectD;
        iColsBegin = qPoint.x() - iRectR - iRectD;
        break;
    case 3:
        iRowBegin =  qPoint.y() - iRectR + 2 * iRectD;
    ...
    CvRect cvRect;
    cvRect.x = iRowBegin,cvRect.y = iColsBegin,cvRect.width = iRectR * 2,cvRect.height = iRectR + iRectD * 3;
    return cvRect;
}
