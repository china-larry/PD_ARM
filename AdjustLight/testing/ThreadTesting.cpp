...
//    qDebug() << __FUNCTION__ << __LINE__;
    ...
    qDebug() << "m_Params.TCup_pas.TCupBasicSaveParams.Light3 = " << m_Params.TCup_pas.TCupBasicSaveParams.Light3;
...
void ThreadTesting::_SLotReceiveQRCodeInfo(QRCodeInfo sInfoQRCodeStruct)
{
    ...
    {
        CHidCmdThread::GetInstance()->AddRotateMotorCmd(10/*SPEED_MOTOR*/, 1024 + 340 + iQRCodePosition, MOVE_FORWARD);
        HIDOpertaionUtility::GetInstance()->SetDeviceOperate(true);
        while (HIDOpertaionUtility::GetInstance()->GetDeviceOperateStates())
        {
            QApplication::processEvents();
...
* @return
*/
void ThreadTesting::_InitLocateLight()
{
    HIDOpertaionUtility::GetInstance()->SetDeviceOperate(true);
...
* @return
*/
void ThreadTesting::_CloseLocateLight()
{
    HIDOpertaionUtility::GetInstance()->SetDeviceOperate(true);
    CHidCmdThread::GetInstance()->AddCmdWithoutCmdData(ProtocolUtility::sm_kiCmdCloseAllLed);
    while (HIDOpertaionUtility::GetInstance()->GetDeviceOperateStates())
...
* @return
*/
void ThreadTesting::_InitAnalyseLight()
{ 
...
//    {
//        qDebug() << "m_Params.TCup_pas.TCupBasicSaveParams.Light3 = " << m_Params.TCup_pas.TCupBasicSaveParams.Light3;
//        CHidCmdThread::GetInstance()->AddOpenLedCmd(3,m_Params.TCup_pas.TCupBasicSaveParams.Light3);
//    }
//    else if(m_qrCodeInfo.eTypeCup == TypeSCup10)
    {
        qDebug() << "m_Params.CubeCup_pas.Light3 = " << m_Params.TCup_pas.TCupBasicSaveParams.Light5/*m_Params.TCup_pas.TCupBasicSaveParams.Light3*/;
...
* @return
*/
void ThreadTesting::_CloseAnalyseLight()
{
    HIDOpertaionUtility::GetInstance()->SetDeviceOperate(true);
    CHidCmdThread::GetInstance()->AddCmdWithoutCmdData(ProtocolUtility::sm_kiCmdCloseAllLed);
    while (HIDOpertaionUtility::GetInstance()->GetDeviceOperateStates())
                ...
                if(_FindFirstItem(strPath,TYPE_FIRST_ITEM, strLocateImagePath) == 0)            //寻找首个项目
                {
                    ...
                    else */                                                   //寻找到首个项目，不是定位条。测试并上报结果
                    {
                        ...
                        //if(bExist)
                        {
                            ...
                            qDebug() << "sResultDataStruct.iTValue = " << sResultDataStruct.iTValue;

                            double dValueTC = (double)sResultDataStruct.iTValue / (double)sResultDataStruct.iCValue;
                            if((m_iIndexMovement - 1) < m_qrCodeInfo.listProject.count())
                            {
                                InfoProject sProjectDataStruct = m_qrCodeInfo.listProject.at(m_iIndexMovement - 1);
                                qDebug() << "dValueTC = " << dValueTC << "  ksProjectDataStruct.dThresholdUp = " << sProjectDataStruct.dThresholdUp;
                                if(sResultDataStruct.iCValue < MIN_VALUE_CLINE)//分量加和 小于1000C线 非法
                                {
                                    qDebug() << "sResultDataStruct.iCValue = " << sResultDataStruct.iCValue;
                                ...
                                else if(sResultDataStruct.strResult == "Error")
                                {
                                    sResultDataStruct.strResult = "Error";
                                    sResultDataStruct.strControlLine = tr("invalid");
                                }
                                else if (dValueTC < sProjectDataStruct.dThresholdUp)
                                {
 ...
 * @return 返回测试结果
 */
TestResultData ThreadTesting::_ReceivePicPathTCup(QString strPath, QString strLocateImagePath, bool &bExist,bool bIsFirstTest)
{
...
//    qDebug() << __FILE__ << "IndexProject:" << m_iIndexProject;
    ...
    sResultDataStruct.strResultImagePath = "";
 ...
 * @return 返回结果
 */
int ThreadTesting::_UprightProjectionH(QString strImgPath)
{
    try
    {
...
bool ThreadTesting::GetReadDevParamstates()
{
...
//    qDebug() << "iLocationCLineMid = " << iLocationCLineMid;
...
{
    double dValueTC = (double)sResultDataStruct.iTValue / (double)sResultDataStruct.iCValue;
    qDebug() << "dValueTC = " << dValueTC << "  ksProjectDataStruct.dThresholdUp = " << ksProjectDataStruct.dThresholdUp;
...
//        qDebug() << iUprightProjectionList.at(x);
...
//     IplImage* src = cvLoadImage((const char *)strImgPath.toLocal8Bit(),0);//导入图片
//     qint16 iPos1 = strImgPath.lastIndexOf(".");
//     QString strDesImage1 = strImgPath.insert(iPos1,"G");
//     cvSaveImage((const char *)strDesImage1.toLocal8Bit(),src);
...
int ThreadTesting::_GetMinLocation(const int * kpSumLineWidth,int iArrLength)
{
    int iMinLocateM = 0;
 ...
 * @return
 */
int ThreadTesting::_GetTLineLoction(const int * pDataArr, int iOrgCenterxLocation,int iLineWidth)
{
 ...
 * @return
 */
bool ThreadTesting::_GetRealLine(int * pDataArr,int iLineCenterX,int iPicWide,int iLineWidth)
{
 ...
 * @return 面积积分
 */
int ThreadTesting::_ImageAnalysisProcess(int * pDataArr, int iOrgLineCenterX, int iPicWide,int iLineWidth)
{
#if 1
...
void ThreadTesting::_SlotReceiveDevParams(DevConfigParams devConfigParams)
{
...
int ThreadTesting::_FindFirstItem(QString strPath, ENUM_LOCATION_TYPE type, QString &strLocateImagePath)
{
    QImage qTarImg;
...
int ThreadTesting::_UprightProjectionHS(QString strImgPath)
{
    QImage* img=new QImage;
    if(!(img->load(strImgPath))) //加载图像
    {
...
QString ThreadTesting::AdjustImage(QString srcImage)
{
...
QVector<CvRect> ThreadTesting::GetTestBarRect(QString strImagePath)
{
