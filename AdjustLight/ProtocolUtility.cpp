...
quint32 g_iUpgradeAppDataSum = 0;
quint32 g_iUpgradeAppDataByte = 0;
quint32  g_iUpgradeAppDataPage = 0;
...
QVector<QByteArray> ProtocolUtility::GetWriteParamFromDevCmd(DevConfigParams sDevConfigParams)
{
    ...
    qDebug() << "GetWriteParamFromDevCmd lenOfParams = " << lenOfParams;
    quint8 iParamStructData[sizeof(DevConfigParams)];    
    memset(&iParamStructData, 0, lenOfParams);
    memcpy(&iParamStructData, &sDevConfigParams, lenOfParams);

//    quint32 iSum = 0;
//    for(int iPos = 0;iPos < lenOfParams - 4;iPos++)
//    {
//        //qDebug() << "iParamStructData[iPos]545 = " << iParamStructData[iPos];
//        iSum += iParamStructData[iPos];
//    }
//    qDebug() << "iSum = " << iSum;
//    qDebug() << "sizeof(quint32) = " << sizeof(quint32);
//    iParamStructData[lenOfParams - 4] = (iSum >> 24) & 0x000000FF;
//    iParamStructData[lenOfParams - 3] = (iSum >> 16) & 0x000000FF;
//    iParamStructData[lenOfParams - 2] = (iSum >> 8) & 0x000000FF;
//    iParamStructData[lenOfParams - 1] = iSum & 0x000000FF;
...
//    UpgradeFile upgradeFile;//升级文件
    if(qSubControlFileInfo.exists())
    {
        if(ENUM_UPGRADE_ERR_OK == upgradeFile.parseUpgradeFile(strFilePath))
        {
            ...
            int iPkgSize = upgradeFile.getDataLen() / kiUpgragePackageDataLen;
            if(upgradeFile.getDataLen() % kiUpgragePackageDataLen)
            {
                ++iPkgSize;
            }

            g_iUpgradeAppDataSum = upgradeFile.getSum();
            g_iUpgradeAppDataByte = upgradeFile.getDataLen();
            g_iUpgradeAppDataPage = iPkgSize;
            ...
            for(; iPkgIndex<=iPkgSize; iPkgIndex++)
            {
                ...
                qOutDataStream<<quint8(sm_kiCmdUpgradeAppData/256)<<quint8(sm_kiCmdUpgradeAppData)//命令类型
                  <<quint8(iPkgIndex/256)<<quint8(iPkgIndex);//
                for(int i=0;i<kiUpgragePackageDataLen;i++)
                {
                    int index = (iPkgIndex -1)*kiUpgragePackageDataLen +i;
                    if(index<iDataLen)
                    {
                        qOutDataStream<< (qint8)upgradeFile.getRealData().at(index);
                    }
                    else
                    {
                        qOutDataStream << (qint8)0xFF;//固定数据字节为50，最后数据包不足够用零补足
                    }
                }
...
QByteArray ProtocolUtility::GetUpgradeAppDataOverCmd()
{
    ...
    qOutDataStream<<quint8(sm_kiCmdUpgradeAppSendDataOver/256)<<quint8(sm_kiCmdUpgradeAppSendDataOver)//命令类型
      ...
      <<quint8(g_iUpgradeAppDataSum >> 24)<<quint8(g_iUpgradeAppDataSum >> 16)<<quint8(g_iUpgradeAppDataSum >> 8)<<quint8(g_iUpgradeAppDataSum)
      <<quint8(g_iUpgradeAppDataByte >> 24)<<quint8(g_iUpgradeAppDataByte >> 16)<<quint8(g_iUpgradeAppDataByte >> 8)<<quint8(g_iUpgradeAppDataByte)
      <<quint8(g_iUpgradeAppDataPage >> 8) << quint8(g_iUpgradeAppDataPage);
...
//写电机补偿步数命令
QByteArray ProtocolUtility::GetWriteRotateCompenSept(QString strRotateCompenSept)
{
    //写仪器序列号
    //命令类型(2byte)		命令参数(2byte)    数据(3byte)
    // 0x0011		    0x0001		      strRotateCompenSept
    QByteArray dataByteArray;
    QDataStream qOutDataStream(&dataByteArray,QIODevice::ReadWrite);
    DealWithCmdHead(qOutDataStream);//填充头部公共部分 设备地址+临时帧长度

    qOutDataStream<<quint8(sm_kiCmdWriteRotateCompenSeptToDev/256)<<quint8(sm_kiCmdWriteRotateCompenSeptToDev)//命令类型
      <<quint8(0x00)<<quint8(0x01);//

    bool bIsok = false;

    int iValue = strRotateCompenSept.toInt(&bIsok,10);
    if(bIsok == true)
    {
        qOutDataStream << (quint8)0x00 << quint8(iValue/256) << quint8(iValue);
    }
    else
    {
        qOutDataStream << (quint8)0x00 << (quint8)0x00 << (quint8)0x00;
    }
