...
bool LineFinder::_crossPoint(cv::Vec4i vLine1Vec, cv::Vec4i vLine2Vec, cv::Point2d& qPoint)
{
    bool isFind = true;
...
cv::Point2d LineFinder::getCenterPoint(std::vector<cv::Point2d> pointVector)
{
    cv::Point2d centerPoint(0,0);
    foreach (cv::Point2d point, pointVector) {
...
void LineFinder::_cutImage(QString strImagePath)
{
    if(strImagePath == "")
    {
         QMessageBox::warning(NULL, QObject::tr("warning"), QObject::tr("Faile to get image!"), QMessageBox::Ok, QMessageBox::Ok);
         return;
    }
    ...
    if(cvImageSize.width < cSize.width && cvImageSize.height < cSize.height)
    {
        return;
    }
    cvSetImageROI(pSrc, cvRect((cvImageSize.width-cSize.width)/2,
                              (cvImageSize.height-cSize.height)/2 - 200,
                              cSize.width,
                              cSize.height));
...
LocationData LineFinder::findCenterPointAndCrossPoints(QString strImagePath)
{
    ...
    cv::Mat mImage = cv::imread(kstrLocationImagePath.toLatin1().data());
    cv::Mat mResult;
    cv::cvtColor (mImage,mResult,CV_BGRA2GRAY);
    cv::Mat contours;
    cv::Canny (mResult,contours,50/*125*/,150/*350*/);  //边缘检测
    //cv::Canny (mResult,contours,125,350);  //边缘检测
    LineFinder finder;
    ...
    sLocationData.centerPoint.y = sLocationData.centerPoint.y + (kiOriginalImageHight - mImage.rows) / 2 - 200;
