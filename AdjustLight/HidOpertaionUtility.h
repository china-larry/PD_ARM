...
static const quint8  g_skiCmdLen = 64;//下位机配置HID每个收/发包数据都是64字节
static const quint16 g_skiUsbVid = 0x0483;
static const quint16 g_skiUsbPid = 0x5750;
...
public:
    ...
    //HID读取数据
    unsigned int HIDRead(quint8 *pRecvDataBuf, int iDelaytime);
    ...
    */
    ...
    bool GetHIDReadStates();

    DevConfigParams GetDevparams();

    bool IsReadDevParames();
...
private:
    ...
    volatile bool m_AckResult;//ACK结果
    volatile bool m_IsWaitForAck;//是否正在等待ACK,用于读写两个线程的信号同步
    volatile bool m_ReturnResult;//结果
    ...
    bool m_bIsUpdateFlag;  //仪器升级标志
    QString m_strDevSerialNumber; //仪器序列号
    bool m_bIsUSBConnect;  //USB是否连接

    static const int sm_kiAckTimeOutSecond = 5;

    bool m_bIsReadDevParams;
};
